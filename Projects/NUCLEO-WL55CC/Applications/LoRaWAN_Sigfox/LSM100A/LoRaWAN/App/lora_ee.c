...
/* Private variables ---------------------------------------------------------*/
EE_LoRa_var_t EE_LoRa_var[CFG_EE_BANK1_SIZE ? 2 : 1];
...
static void EE_LoRa_Reset(EE_LoRa_var_t *pv, uint32_t address, uint8_t nb_pages);

static int32_t EE_LoRa_Recovery(EE_LoRa_var_t *pv);

static int32_t EE_LoRa_Transfer(EE_LoRa_var_t *pv, uint16_t addr, uint32_t page);

static int32_t EE_LoRa_WriteEl(EE_LoRa_var_t *pv, uint16_t addr, uint32_t data);

static int32_t EE_LoRa_ReadEl(const EE_LoRa_var_t *pv,
                         uint16_t addr, uint32_t *data, uint32_t page);

static int32_t EE_LoRa_SetState(const EE_LoRa_var_t *pv, uint32_t page, uint32_t state);

static uint32_t EE_LoRa_GetState(const EE_LoRa_var_t *pv, uint32_t page);
...
int32_t EE_LoRa_Init(int32_t format, uint32_t base_address)
{
  /* USER CODE BEGIN EE_LoRa_Init_1 */

  /* USER CODE END EE_LoRa_Init_1 */
  int32_t status;
  ...
  if (format)
  {
    ...
    {
      status = EE_LoRa_SetState(&EE_LoRa_var[1], 0, EE_STATE_ACTIVE);
    }
  ...
  if (CFG_EE_BANK1_SIZE && (status == EE_OK))
  {
    status = EE_LoRa_Recovery(&EE_LoRa_var[1]);
  }

  return status;
  /* USER CODE BEGIN EE_LoRa_Init_2 */

  /* USER CODE END EE_LoRa_Init_2 */
}

int32_t EE_LoRa_Read(int32_t bank, uint16_t addr, uint32_t *data)
{
  /* USER CODE BEGIN EE_LoRa_Read_1 */

  /* USER CODE END EE_LoRa_Read_1 */
  EE_LoRa_var_t *pv = &EE_LoRa_var[CFG_EE_BANK1_SIZE && bank];

  /* Read element starting from active page */
  return EE_LoRa_ReadEl(pv, addr, data, pv->current_write_page);
  /* USER CODE BEGIN EE_LoRa_Read_2 */

  /* USER CODE END EE_LoRa_Read_2 */
}

int32_t EE_LoRa_Write(int32_t bank, uint16_t addr, uint32_t data)
{
  /* USER CODE BEGIN EE_LoRa_Write_1 */

  /* USER CODE END EE_LoRa_Write_1 */
  EE_LoRa_var_t *pv = &EE_LoRa_var[CFG_EE_BANK1_SIZE && bank];
  uint32_t page;
  ...
  /* USER CODE BEGIN EE_LoRa_Write_2 */

  /* USER CODE END EE_LoRa_Write_2 */
}

int32_t EE_LoRa_Clean(int32_t bank, int32_t interrupt)
{
  /* USER CODE BEGIN EE_LoRa_Clean_1 */

  /* USER CODE END EE_LoRa_Clean_1 */
  EE_LoRa_var_t *pv = &EE_LoRa_var[CFG_EE_BANK1_SIZE && bank];
  uint32_t page;
  ...
  return EE_OK;
  /* USER CODE BEGIN EE_LoRa_Clean_2 */

  /* USER CODE END EE_LoRa_Clean_2 */
}
...
static int32_t EE_LoRa_Recovery(EE_LoRa_var_t *pv)
{
  /* USER CODE BEGIN EE_LoRa_Recovery_1 */

  /* USER CODE END EE_LoRa_Recovery_1 */
  uint32_t page;
  uint32_t first_page;
  uint32_t state;
  uint32_t prev_state;
  ...
  return EE_STATE_ERROR;
  /* USER CODE BEGIN EE_LoRa_Recovery_2 */

  /* USER CODE END EE_LoRa_Recovery_2 */
}

static int32_t EE_LoRa_Transfer(EE_LoRa_var_t *pv, uint16_t addr, uint32_t page)
{
  /* USER CODE BEGIN EE_LoRa_Transfer_1 */

  /* USER CODE END EE_LoRa_Transfer_1 */
  uint32_t state;
  uint32_t var;
  uint32_t data;
  uint32_t last_page;
  ...
  /* USER CODE BEGIN EE_LoRa_Transfer_2 */

  /* USER CODE END EE_LoRa_Transfer_2 */
}

static int32_t EE_LoRa_WriteEl(EE_LoRa_var_t *pv, uint16_t addr, uint32_t data)
{
  /* USER CODE BEGIN EE_LoRa_WriteEl_1 */

  /* USER CODE END EE_LoRa_WriteEl_1 */
  uint32_t page;
  uint32_t flash_addr;
  uint64_t el;
  ...
  return EE_OK;
  /* USER CODE BEGIN EE_LoRa_WriteEl_2 */

  /* USER CODE END EE_LoRa_WriteEl_2 */
}

static int32_t EE_LoRa_ReadEl(const EE_LoRa_var_t *pv,
                         uint16_t addr, uint32_t *data, uint32_t page)
{
  /* USER CODE BEGIN EE_LoRa_ReadEl_1 */

  /* USER CODE END EE_LoRa_ReadEl_1 */
  uint32_t flash_addr;
  uint32_t offset;
  uint64_t el;
  ...
  /* USER CODE BEGIN EE_LoRa_ReadEl_2 */

  /* USER CODE END EE_LoRa_ReadEl_2 */
}

static int32_t EE_LoRa_SetState(const EE_LoRa_var_t *pv, uint32_t page, uint32_t state)
{
  /* USER CODE BEGIN EE_LoRa_SetState_1 */

  /* USER CODE END EE_LoRa_SetState_1 */
  uint32_t flash_addr;
  ...
  return EE_OK;
  /* USER CODE BEGIN EE_LoRa_SetState_2 */

  /* USER CODE END EE_LoRa_SetState_2 */
}

static uint32_t EE_LoRa_GetState(const EE_LoRa_var_t *pv, uint32_t page)
{
  /* USER CODE BEGIN EE_LoRa_GetState_1 */
